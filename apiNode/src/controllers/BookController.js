...
class BookController {
  static async listBooks(req, res, next) {
    //= No conttroller é o primeiro middleware chamado, depois vem os proximos
    try {
      const searchAllBooks = book.find({});

      // = Isso salva esse dado em uma variavel na requisição,e assim no proximo middleware será possivel pegar esse dado
      req.result = searchAllBooks;

      // = Pula para o proximo middleware cadastrado no controllador
      next();
    } catch (error) {
      next(error);
    }
